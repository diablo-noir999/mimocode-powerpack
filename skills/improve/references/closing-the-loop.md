...
Spawn **one** `general` subagent with `isolation: "worktree"`. Use the `general` agent type — it has write access and multi-step reasoning needed for implementation work. Executor model: default `sonnet`; use what the user named if they named one (`execute 003 haiku`).
