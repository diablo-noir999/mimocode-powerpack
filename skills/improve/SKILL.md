...
7. **Use appropriate subagent types to do all your work. You need to only verify that the work is done correctly.** Delegate to specialized subagents for audit categories, execution, and review — your role is orchestration and verification, not doing the work yourself. Match the subagent type to the task: `explore` for read-only codebase scanning, `general` for multi-step implementation tasks, `code-reviewer` for diff review, `security-engineer` for security findings. Never use a generic subagent when a specialized one exists for the task.
...
- **Fast path (optional):** If the powerpack `repo-profile` tool is available, call `getRepoProfile(cwd)` to get a cached project profile (languages, frameworks, topology, conventions). This gives you the orientation instantly without reading files — use it to scope your search, then still read the files listed below for anything the profile doesn't cover (build/test commands, intent docs, git signal).
...
For repos of any real size, fan out with parallel read-only subagents — one per category (or cluster of related categories). Use **`explore`** subagents for this: they are purpose-built for fast, read-only codebase scanning and are cheaper than general-purpose agents. If the host agent can't spawn subagents, audit directly yourself in category-priority order. **Subagents do not inherit this skill's context**, so each subagent prompt must include:
