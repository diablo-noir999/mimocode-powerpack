...
function runBunPragma(db: BunSqliteDatabase, sql: string): void {
  try {
    ...
    console.debug("[sqlite] PRAGMA failed:", e instanceof Error ? e.message : e)
  }
}
  ...
  return {
    ...
    close(): void {
      try {
        ...
        console.debug("[sqlite] close failed:", e instanceof Error ? e.message : e)
      }
    },
  };
}
...
async function importNodeSqlite(): Promise<NodeSqliteModule | null> {
  try {
    ...
    console.debug("[sqlite] node:sqlite not available:", e instanceof Error ? e.message : e)
    return null;
  }
}
