...
import { clampPercent, coerceFiniteNumber } from "./format-utils.js";
...
function parseResetTimeIso(data: Record<string, unknown>): string | undefined {
  ...
  for (const key of ["reset_in", "resetIn", "ttl"]) {
    const seconds = coerceFiniteNumber(data[key]);
  ...
  if (window !== null && typeof window === "object") {
    ...
    const windowSeconds = coerceFiniteNumber(w.duration);
...
function buildLimitLabel(
  ...
  const duration = coerceFiniteNumber(window.duration ?? item.duration ?? detail.duration);
...
function toUsageRow(
  ...
  const limit = coerceFiniteNumber(data.limit);
  let used = coerceFiniteNumber(data.used);

  if (used === undefined) {
    const remaining = coerceFiniteNumber(data.remaining);
