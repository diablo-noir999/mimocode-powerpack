...
import { clampPercent, coerceFiniteNumber, fmtUsdAmount } from "./format-utils.js";
...
function normalizeUsageWindow(
  ...
  const used = coerceFiniteNumber(value.used);
  const remainingRaw = coerceFiniteNumber(value.remaining);
  const limitFromResponse = getFinitePositiveNumber(limitValue);
  const percentUsed = coerceFiniteNumber(value.percentUsed);
